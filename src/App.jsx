...
function CastingAppInner({ authUser }) {
  ...
  useEffect(() => {
    ...
    doSync(); // initial sync
    const interval = setInterval(doSync, 3000); // poll every 10s
  ...
  const filteredProfiles = currentProfiles.filter(p => {
    ...
    const sel = { choice: getChoice(p.id), ...(state.selections[p.id] || {}) };
              ...
              <div>
                {(() => {
                  ...
                  const roleProfiles = (state.profiles[currentRole] || []).map(p => ({
                    ...p, _role: currentRole, _selection: { choice: getChoice(p.id), ...(state.selections[p.id] || {}) },
                  }));
            ...
            <div>
              {(() => {
                ...
                const renderProfileCard = (profile, i) => {
                  ...
                  const sel = { choice: getChoice(profile.id), ...(state.selections[profile.id] || {}) };
                ...
                {profiles.map(p => (
                  <div key={p.id} style={{ background: "#111114", borderRadius: 14, border: "1px solid #1e1e22", overflow: "hidden" }}>
                    ...
                    <div style={{ padding: "14px" }}>
                      ...
                      {getChoice(p.id) && (
                        <div style={{ marginTop: 8 }}><SelectionBadge selection={{ choice: getChoice(p.id), ...(state.selections[p.id] || {}) }} /></div>
                      )}
