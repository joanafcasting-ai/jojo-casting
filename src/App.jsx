...
function CastingAppInner({ authUser }) {
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        ...
        if (cleanState._shareCode) {
          try {
            // Merge with existing shared data to preserve guest votes
            const existing = await window.storage.get(`shared:${cleanState._shareCode}`, true);
            let sharedData = cleanState;
            if (existing?.value) {
              const existingData = JSON.parse(existing.value);
              sharedData = { ...cleanState,
                _guestVotes: existingData._guestVotes || cleanState._guestVotes || {},
                _guestCastingVotes: existingData._guestCastingVotes || cleanState._guestCastingVotes || {},
                _guestComments: existingData._guestComments || cleanState._guestComments || {},
                _guestUpdatedAt: existingData._guestUpdatedAt || cleanState._guestUpdatedAt,
              };
            }
  ...
  // Also resync shared data when director opens shared project — auto-poll every 10s
  useEffect(() => {
    ...
    const doSync = async () => {
      ...
      if (updated) {
        setState(prev => ({
          ...prev,
          _guestVotes: updated._guestVotes || prev._guestVotes || {},
          _guestCastingVotes: updated._guestCastingVotes || prev._guestCastingVotes || {},
          _guestComments: updated._guestComments || prev._guestComments || {},
        }));
      }
    };
    doSync(); // initial sync
    const interval = setInterval(doSync, 10000); // poll every 10s
  ...
  const handleExportBackup = async () => {
    ...
    try {
      ...
      for (const key of keys.keys || []) {
        try {
          const data = await window.storage.get(key);
          if (data?.value) projects.push({ key, data: JSON.parse(data.value) });
        } catch (e) {}
      }
      // Also get castingFile
      let castingFile = null;
      try {
        const cf = await window.storage.get("castingFile");
        if (cf?.value) castingFile = JSON.parse(cf.value);
      } catch (e) {}
      ...
      a.href = url;
      a.download = `casting-backup-${new Date().toISOString().split("T")[0]}.json`;
      a.click();
          ...
          <div style={{ maxWidth: 900, margin: "0 auto", padding: "40px 24px" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16, marginBottom: 20, padding: "14px 18px", background: "#111114", borderRadius: 12, border: "1px solid #1e1e22" }}>
              ...
              <button onClick={handleExportBackup}
                ...
                style={{
                  padding: "8px 16px", background: "rgba(96,165,250,0.08)", border: "1px solid rgba(96,165,250,0.2)",
                ...
                {backupStatus === "exporting" ? "⏳ Export..." : backupStatus === "done" ? "✓ Téléchargé !" : "📥 Sauvegarder"}
              </button>
            </div>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                {state._shareCode && state._guestVotes && Object.keys(state._guestVotes).length > 0 && (
                  <span style={{ fontSize: 9, padding: "3px 8px", background: "rgba(96,165,250,0.1)", border: "1px solid rgba(96,165,250,0.2)", borderRadius: 6, color: "#60a5fa", fontWeight: 700 }}>
                    {Object.keys(state._guestVotes).length} vote{Object.keys(state._guestVotes).length !== 1 ? "s" : ""} invité
                  </span>
                )}
                <div style={{ width: 1, height: 20, background: "#2a2a2e" }} />
            <button onClick={() => setLightTheme(!lightTheme)}
          ...
          <nav style={{
            ...
            <div style={{ padding: "0 8px", marginBottom: 20, display: "flex", flexDirection: "column", gap: 2 }}>
              {[
                ...
                { key: "contacts", icon: "📇", label: "Contacts", color: "#60a5fa", show: true, badge: (() => { const all = Object.values(state.profiles).flat(); return all.filter(p => state.selections[p.id]?.choice === "yes" || state.selections[p.id]?.choice === "maybe" || state.selections[p.id]?.choice === "no").length; })() },
                    ...
                    <div key={profile.id} style={{ animation: `fadeIn 0.3s ease ${i * 0.05}s both`, position: "relative" }}>
                      ...
                      {viewMode === "realisateur" ? (
                        ...
                        <div style={{ position: "relative" }}>
                          ...
                          {/* Guest votes indicator */}
                          {(state._guestVotes?.[profile.id] || state._guestComments?.[profile.id]?.length > 0) && (
                              ...
                              <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                                {state._guestVotes?.[profile.id] && (
                                  <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 4, fontWeight: 700,
                                    background: state._guestVotes[profile.id].choice === "yes" ? "rgba(34,197,94,0.1)" : state._guestVotes[profile.id].choice === "no" ? "rgba(239,68,68,0.1)" : "rgba(245,158,11,0.1)",
                                    color: state._guestVotes[profile.id].choice === "yes" ? "#22c55e" : state._guestVotes[profile.id].choice === "no" ? "#ef4444" : "#f59e0b",
                                  }}>
                                    👥 Invité : {state._guestVotes[profile.id].choice === "yes" ? "OUI" : state._guestVotes[profile.id].choice === "no" ? "NON" : "PEUT-ÊTRE"}
                                  </span>
                                )}
                              </div>
                              {state._guestComments?.[profile.id]?.map((c, ci) => (
                                <div key={ci} style={{ fontSize: 10, color: "#888", marginTop: 4, fontStyle: "italic" }}>
                                  💬 {c.text}
                                </div>
                              ))}
                            </div>
...
function GuestView({ shareCode, project, password }) {
  ...
  const voteButton = (profileId, choice, label, color, isActive, onClick) => (
    <button onClick={() => onClick(profileId, choice)} style={{
      padding: "8px 18px", borderRadius: 8, cursor: "pointer", fontSize: 12, fontWeight: 700,
      fontFamily: "inherit", border: isActive ? `2px solid ${color}` : "1px solid #2a2a2e",
      background: isActive ? `${color}18` : "#111114", color: isActive ? color : "#888",
      transition: "all 0.2s",
    }}>
      {label}
    </button>
  ...
  const renderProfileCard = (profile, showCastingVotes = false) => {
    ...
    return (
      <div key={profile.id} style={{
        ...
        borderColor: v?.choice === "yes" ? "rgba(34,197,94,0.3)" : v?.choice === "no" ? "rgba(239,68,68,0.3)" : v?.choice === "maybe" ? "rgba(245,158,11,0.3)" : "#1e1e22",
      }}>
        {/* Header */}
        <div onClick={() => setExpandedProfile(isExpanded ? null : profile.id)}
          style={{ display: "flex", alignItems: "center", gap: 14, padding: "16px 18px", cursor: "pointer" }}>
          {profile.photo ? (
            <img src={profile.photo} style={{ width: 56, height: 56, borderRadius: 12, objectFit: "cover" }} />
          ) : (
            <div style={{ width: 56, height: 56, borderRadius: 12, background: "#1e1e22", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, color: "#555" }}>
              {(profile.firstName || "?")[0]}
            </div>
          )}
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 11, color: "#888", marginTop: 2 }}>
              {[profile.age && `${profile.age} ans`, profile.city, profile.agency].filter(Boolean).join(" · ")}
            </div>
          ...
          <span style={{ fontSize: 16, color: "#555", transition: "transform 0.2s", transform: isExpanded ? "rotate(180deg)" : "" }}>▾</span>
        </div>

        {/* Expanded details */}
        {isExpanded && (
          <div style={{ padding: "0 18px 18px", borderTop: "1px solid #1a1a1e" }}>
            {/* Profile details */}
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap", padding: "14px 0", fontSize: 12, color: "#999" }}>
              {profile.height && <span>📏 {profile.height}</span>}
            ...
            {profile.selftapeLinks?.filter(l => l).length > 0 && (
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 10, color: "#555", fontWeight: 600, textTransform: "uppercase", marginBottom: 6 }}>▶ Liens vidéo</div>
                {profile.selftapeLinks.filter(l => l).map((link, i) => (
                  <a key={i} href={link} target="_blank" rel="noreferrer" style={{ display: "block", color: "#60a5fa", fontSize: 12, marginBottom: 4, textDecoration: "none" }}>
            ...
            {/* Selftape videos */}
            {profile.selftapeVideos?.length > 0 && (
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 10, color: "#555", fontWeight: 600, textTransform: "uppercase", marginBottom: 6 }}>🎬 Selftapes</div>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  {profile.selftapeVideos.map((video, i) => (
                    <button key={i} onClick={() => setPlayingVideo(video)}
                      style={{ padding: "8px 14px", background: "#0c0c0e", border: "1px solid #2a2a2e", borderRadius: 8, cursor: "pointer", fontSize: 11, color: "#60a5fa", fontFamily: "inherit" }}>
                      ▶ {video.name || `Vidéo ${i + 1}`}
                    </button>
                  ))}
                </div>
              </div>
            ...
            {/* Director's comment (casting tab) */}
            {showCastingVotes && session.comment && (
              <div style={{ padding: "10px 14px", background: "rgba(201,164,74,0.06)", border: "1px solid rgba(201,164,74,0.15)", borderRadius: 8, marginBottom: 12 }}>
                <div style={{ fontSize: 9, color: "#c9a44a", fontWeight: 600, textTransform: "uppercase", marginBottom: 4 }}>💬 Note du Casting Director</div>
                <div style={{ fontSize: 12, color: "#ccc", lineHeight: 1.5 }}>{session.comment}</div>
              </div>
            )}

            {/* Director's notes on profile */}
            {profile.notes && (
              <div style={{ padding: "10px 14px", background: "rgba(255,255,255,0.02)", border: "1px solid #1e1e22", borderRadius: 8, marginBottom: 12 }}>
                <div style={{ fontSize: 9, color: "#888", fontWeight: 600, textTransform: "uppercase", marginBottom: 4 }}>📝 Notes</div>
                <div style={{ fontSize: 12, color: "#ccc", lineHeight: 1.5 }}>{profile.notes}</div>
              </div>
            )}
            ...
            {profileComments.length > 0 && (
              <div style={{ marginBottom: 12 }}>
                <div style={{ fontSize: 9, color: "#555", fontWeight: 600, textTransform: "uppercase", marginBottom: 6 }}>Vos commentaires</div>
                {profileComments.map((c, i) => (
                  <div key={i} style={{ padding: "8px 12px", background: "#0c0c0e", borderRadius: 8, marginBottom: 4, fontSize: 12, color: "#ccc" }}>
                    {c.text}
                    <span style={{ fontSize: 9, color: "#555", marginLeft: 8 }}>{new Date(c.at).toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" })}</span>
                  </div>
                ))}
              </div>
            )}
            ...
            <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
              <input
                ...
                style={{ flex: 1, padding: "10px 14px", background: "#0c0c0e", border: "1px solid #2a2a2e", borderRadius: 8, color: "#e0e0e0", fontSize: 12, fontFamily: "'DM Sans',sans-serif", outline: "none" }}
                onFocus={e => e.target.style.borderColor = "#c9a44a"}
                onBlur={e => e.target.style.borderColor = "#2a2a2e"}
              />
              <button onClick={() => addComment(profile.id)} style={{ padding: "10px 14px", background: "rgba(201,164,74,0.1)", border: "1px solid rgba(201,164,74,0.2)", borderRadius: 8, cursor: "pointer", fontSize: 12, color: "#c9a44a", fontWeight: 700, fontFamily: "inherit" }}>
                Envoyer
              </button>
            </div>
            ...
            <div style={{ display: "flex", gap: 8, justifyContent: "center" }}>
              {showCastingVotes ? (
                <>
                  {voteButton(profile.id, "yes", "✓ OUI", "#22c55e", castingVotes[profile.id]?.choice === "yes", castingVote)}
                  {voteButton(profile.id, "no", "✕ NON", "#ef4444", castingVotes[profile.id]?.choice === "no", castingVote)}
                </>
              ) : (
                <>
                  {voteButton(profile.id, "yes", "✓ OUI", "#22c55e", votes[profile.id]?.choice === "yes", vote)}
                  {voteButton(profile.id, "maybe", "~ PEUT-ÊTRE", "#f59e0b", votes[profile.id]?.choice === "maybe", vote)}
                  {voteButton(profile.id, "no", "✕ NON", "#ef4444", votes[profile.id]?.choice === "no", vote)}
                </>
              )}
            </div>
          </div>
                ...
                <div style={{ animation: "fadeIn 0.3s" }}>
                  ...
                  <div style={{ display: "flex", gap: 12, marginBottom: 16 }}>
                    {[
                      ...
                      { label: "Peut-être", count: roleProfiles.filter(p => votes[p.id]?.choice === "maybe").length, color: "#f59e0b" },
                ...
                <div style={{ animation: "fadeIn 0.3s" }}>
                  ...
                  {finalProfiles.map(p => (
                    <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 14, padding: "16px 18px", background: "#111114", border: "1px solid rgba(34,197,94,0.2)", borderRadius: 14, marginBottom: 10 }}>
                      {p.photo ? (
                        <img src={p.photo} style={{ width: 48, height: 48, borderRadius: 10, objectFit: "cover" }} />
                      ) : (
...
export default function CastingApp() {
  // ===== GUEST SHARE MODE =====
  const [guestShareCode, setGuestShareCode] = useState(null);
  ...
  // Detect ?share=CODE in URL on load
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const code = params.get("share");
    if (code) {
      setGuestShareCode(code.toUpperCase());
  ...
  // If in guest share mode, render guest flow
  if (guestShareCode) {
    const guestStyles = `
      @import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;600;700;800&family=Playfair+Display:wght@700;800&display=swap');
      * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        <>
          ...
          <div style={{ minHeight: "100vh", background: "#0a0a0c", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
            <div style={{ textAlign: "center", animation: "fadeIn 0.5s" }}>
              ...
              <p style={{ fontSize: 13, color: "#666" }}>Ce lien de partage n'est plus valide ou le projet a été supprimé.</p>
            </div>
          </div>
        </>
      );
            ...
            <div style={{ width: "100%", maxWidth: 380, textAlign: "center", animation: "fadeIn 0.5s ease" }}>
              ...
              <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
                <input
                  ...
                  autoFocus
                  style={{
                    flex: 1, padding: "14px 18px", background: "#111114",
    ...
    // Guest authenticated — show GuestView
    return <GuestView shareCode={guestShareCode} project={guestProject} password={guestPasswordInput} />;
  }

  // ===== NORMAL DIRECTOR MODE =====
  ...
  // Check if site was already unlocked (session only, not persisted)
  // Code required every visit
  ...
  useEffect(() => {
    if (siteUnlocked) {
      // Site unlocked — auto-login with fixed user
      const user = {
        id: "joana_casting_director",
        email: "joana@casting.com",
        firstName: "Joana",
