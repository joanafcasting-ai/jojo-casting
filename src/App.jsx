...
import { setCurrentUserId } from "./supabase.js";
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: "14px 16px 16px" }}>
        ...
        {profile.agency && (
          <div style={{ fontSize: 11, color: "#c9a44a", fontWeight: 500, letterSpacing: "0.03em" }}>
            {profile.agency}
          </div>
        )}
...
function ProfileForm({ profile, onSave, onDelete, onClose }) {
  ...
  const handleVideoUpload = useCallback((e) => {
    ...
    files.forEach(file => {
      ...
      videos.push({ data: blobUrl, name: file.name, size: file.size, type: file.type });
  ...
  return (
    <div>
      ...
      <input ref={videoRef} type="file" accept="video/*" multiple style={{ display: "none" }} onChange={handleVideoUpload} />
      ...
      <div style={{ marginBottom: 16 }}>
        <label style={{ display: "block", fontSize: 12, color: "#888", marginBottom: 4, fontWeight: 500, letterSpacing: "0.04em", textTransform: "uppercase" }}>
          Selftapes — Upload ({(form.selftapeVideos || []).length} fichier{(form.selftapeVideos || []).length !== 1 ? "s" : ""})
        </label>
        <div style={{ fontSize: 10, color: "#555", marginBottom: 10, fontStyle: "italic" }}>
          Lecture disponible une fois l'app déployée sur serveur
        </div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          ...
          >
            ...
            <span style={{ fontSize: 10, letterSpacing: "0.03em" }}>Ajouter vidéo</span>
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{
      ...
      <div style={{ padding: "14px 16px 8px" }}>
        ...
        {profile.agency && (
          <div style={{ fontSize: 11, color: "#c9a44a", fontWeight: 500, letterSpacing: "0.03em", marginBottom: 4 }}>
            {profile.agency}
          </div>
        )}
...
function CastingAppInner({ authUser, onLogout }) {
  // ===== PROJECT MANAGEMENT =====
    ...
    saveTimer.current = setTimeout(async () => {
      try {
        ...
        if (cleanState._shareCode) {
          try { await window.storage.set(`shared:${cleanState._shareCode}`, JSON.stringify(cleanState), true); } catch (e) {}
        }
  ...
  const syncFromShared = async (projectId) => {
    try {
      ...
      if (sp._guestUpdatedAt) {
        project.selections = { ...project.selections, ...sp.selections };
        if (sp.castingSessions) project.castingSessions = { ...project.castingSessions, ...sp.castingSessions };
        if (sp.finalSelections) project.finalSelections = { ...project.finalSelections, ...sp.finalSelections };
        await window.storage.set(`project:${projectId}`, JSON.stringify(project));
        return project;
      }
    } catch (e) {}
  ...
  const [viewMode, setViewMode] = useState("director"); // "director" | "production" | "realisateur"
  const [contactModalOpen, setContactModalOpen] = useState(false);
  const [contactingProfile, setContactingProfile] = useState(null);
  const [activeTab, setActiveTab] = useState("roles"); // "roles" | "contacts"
  ...
  // Also resync shared data when director opens shared project
  useEffect(() => {
    if (currentProjectId && state.started && !guestMode && state._shareCode) {
      syncFromShared(currentProjectId).then(updated => {
        if (updated) setState(prev => ({ ...prev, selections: updated.selections || prev.selections, finalSelections: updated.finalSelections || prev.finalSelections, castingSessions: updated.castingSessions || prev.castingSessions }));
      });
    }
  }, [currentProjectId]); // only on load

  const cycleViewMode = () => {
    if (guestMode) return; // locked in realisateur mode
    setViewMode(v => v === "director" ? "production" : v === "production" ? "realisateur" : "director");
  };

  const VIEW_MODES = {
    director: { label: "Vue Director", color: "#c9a44a", icon: "🎬" },
    production: { label: "Vue Production", color: "#60a5fa", icon: "📋" },
    realisateur: { label: "Vue Réalisateur", color: "#a78bfa", icon: "🎥" },
  };
  ...
  const getCastingProfiles = () => {
    ...
    state.roles.forEach(role => {
      (state.profiles[role] || []).forEach(p => {
        const sel = state.selections[p.id]?.choice;
          ...
          <header style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              ...
              <div style={{ display: "flex", gap: 4 }}>
                <button onClick={() => setCastingFileView(false)} style={{
                  padding: "6px 16px", borderRadius: 6, fontSize: 11, fontWeight: 600,
                  fontFamily: "inherit", border: "none", cursor: "pointer",
                  background: !castingFileView ? "rgba(201,164,74,0.12)" : "transparent",
                  color: !castingFileView ? "#c9a44a" : "#555", transition: "all 0.2s",
                }}>🎬 Projets</button>
                <button onClick={() => setCastingFileView(true)} style={{
                  padding: "6px 16px", borderRadius: 6, fontSize: 11, fontWeight: 600,
                  fontFamily: "inherit", border: "none", cursor: "pointer",
                  background: castingFileView ? "rgba(168,85,247,0.12)" : "transparent",
                  color: castingFileView ? "#a855f7" : "#555", transition: "all 0.2s",
                  position: "relative",
                }}>
                  📁 Fichier Casting
              ...
              <div style={{ position: "relative" }}>
                <button onClick={e => {
                  ...
                  style={{
                    ...
                    background: "rgba(201,164,74,0.06)", border: "1px solid #2a2a2e", borderRadius: 8,
                    cursor: "pointer", fontFamily: "inherit",
                  }}>
                  <div style={{
                    width: 26, height: 26, borderRadius: "50%",
                    background: "linear-gradient(135deg, #c9a44a, #b8963a)",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    fontSize: 11, fontWeight: 800, color: "#000",
                  }}>
                    {(authUser.firstName || authUser.email || "U")[0].toUpperCase()}
                  </div>
                  <span style={{ fontSize: 11, color: "#ccc", fontWeight: 600, maxWidth: 100, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                    {authUser.firstName || authUser.email}
                  </span>
                </button>
                <div style={{ display: "none", position: "absolute", top: "100%", right: 0, marginTop: 6, width: 220, background: "#141416", border: "1px solid #2a2a2e", borderRadius: 12, padding: "8px", zIndex: 999 }}>
                  <div style={{ padding: "10px 12px", borderBottom: "1px solid #1e1e22", marginBottom: 4 }}>
                    <div style={{ fontSize: 13, fontWeight: 700, color: "#f0f0f0" }}>{[authUser.firstName, authUser.lastName].filter(Boolean).join(" ")}</div>
                    <div style={{ fontSize: 10, color: "#666", marginTop: 2 }}>{authUser.email}</div>
                    {authUser.provider && authUser.provider !== "email" && (
                      <div style={{ fontSize: 9, color: "#555", marginTop: 3 }}>Connecté via {authUser.provider === "google" ? "Google" : "Apple"}</div>
                    )}
                  </div>
                  <button onClick={onLogout}
                    style={{
                      ...
                      fontSize: 12, color: "#ef4444", fontWeight: 600, transition: "background 0.2s",
                    }}
  ...
  const archivedCount = currentProfiles.filter(p => state.selections[p.id]?.choice === "no").length;
  const filteredProfiles = currentProfiles.filter(p => {
    ...
    const matchesSelection = filterSelection === "all"
      || (filterSelection === "none" && (!sel || !sel.choice))
  ...
  return (
    <>
      ...
      <div className={lightTheme ? "light-wrapper" : ""} style={{ minHeight: "100vh", background: "#0a0a0c" }}>
        ...
        <header style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
            ...
            {guestMode && (
              <span style={{ fontSize: 9, padding: "3px 10px", background: "rgba(168,85,247,0.12)", border: "1px solid rgba(168,85,247,0.25)", borderRadius: 6, color: "#a855f7", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em" }}>
                👁 Vue Réalisateur
              </span>
            )}
            <div style={{ width: 1, height: 20, background: "#2a2a2e" }} />
              ...
              <>
                ...
                <button onClick={async () => {
                  if (state._shareCode) { setShareModalOpen(true); }
                    ...
                    const code = await shareProject(currentProjectId);
                    if (code) { setState(prev => ({ ...prev, _shareCode: code })); setShareModalOpen(true); }
                  }
                }}
                  ...
                  {state._shareCode ? `🔗 ${state._shareCode}` : "🔗 Partager"}
                </button>
                {state._shareCode && (
                  <button onClick={async () => {
                    const updated = await syncFromShared(currentProjectId);
                    if (updated) setState(prev => ({ ...prev, selections: updated.selections || prev.selections, finalSelections: updated.finalSelections || prev.finalSelections }));
                  }}
                    style={{ padding: "5px 10px", background: "rgba(96,165,250,0.08)", border: "1px solid rgba(96,165,250,0.2)", borderRadius: 6, cursor: "pointer", fontSize: 10, fontWeight: 600, fontFamily: "inherit", color: "#60a5fa" }}
                    title="Récupérer les choix du réalisateur">
                    🔄 Sync
                  </button>
                )}
                <div style={{ width: 1, height: 20, background: "#2a2a2e" }} />
            ...
            {!guestMode && (
              <button
                ...
                style={{
                  ...
                  fontSize: 11, fontWeight: 600, fontFamily: "inherit",
                  letterSpacing: "0.04em", textTransform: "uppercase",
                  border: `1px solid ${VIEW_MODES[viewMode].color}44`,
                  background: `${VIEW_MODES[viewMode].color}14`,
                  color: VIEW_MODES[viewMode].color,
                  transition: "all 0.3s",
                }}
              >
                <span style={{
                  width: 7, height: 7, borderRadius: "50%",
                  background: VIEW_MODES[viewMode].color,
                }} />
          ...
          <nav style={{
            ...
            <div style={{ padding: "0 8px", marginBottom: 20, display: "flex", flexDirection: "column", gap: 2 }}>
              {[
                ...
                { key: "contacts", icon: "📇", label: "Contacts", color: "#60a5fa", show: !guestMode, badge: (() => { const all = Object.values(state.profiles).flat(); return all.filter(p => state.selections[p.id]?.choice === "yes" || state.selections[p.id]?.choice === "maybe" || state.selections[p.id]?.choice === "no").length; })() },
                { key: "planning", icon: "📋", label: "Planning", color: "#a855f7", show: !guestMode, badge: state.castingDays.length },
                ...
                { key: "mail", icon: "✉", label: "Gmail", color: "#EA4335", show: !guestMode, badge: (state.emailLog || []).length },
            ...
            {state.roles.map(role => {
              ...
              const yesCount = roleProfiles.filter(p => state.selections[p.id]?.choice === "yes").length;
              const maybeCount = roleProfiles.filter(p => state.selections[p.id]?.choice === "maybe").length;
              const noCount = roleProfiles.filter(p => state.selections[p.id]?.choice === "no").length;
            ...
            {activeTab === "contacts" && (() => {
              ...
              if (isFinal) {
                ...
                rolesWithContacts = state.roles.filter(role => {
                  return (state.profiles[role] || []).some(p => state.selections[p.id]?.choice);
                });
              }
                  ...
                  {state.roles.map(role => {
                    ...
                    if (isFinal) {
                      ...
                      selCount = roleProfiles.filter(p => {
                        const s = state.selections[p.id]?.choice;
                        return s === "yes" || s === "maybe";
                      }).length;
                      noCount = roleProfiles.filter(p => state.selections[p.id]?.choice === "no").length;
                      totalEval = selCount + noCount;
                      const evaluatedProfiles = roleProfiles.filter(p => state.selections[p.id]?.choice);
              ...
              <div>
                ...
                <div style={{ padding: "0 20px", marginBottom: 10 }}>
                  <div style={{ fontSize: 10, letterSpacing: "0.2em", textTransform: "uppercase", color: "#555", fontWeight: 600 }}>
                    {activeTab === "final" ? "Casting définitif" : guestMode ? "Rôles" : "Par rôle"}
                  </div>
                </div>
                {state.roles.map(role => {
                  const roleProfiles = state.profiles[role] || [];
          ...
          <main style={{ flex: 1, padding: "28px 36px" }}>
              ...
              <div>
                {(() => {
                  ...
                  if (isFinal) {
                    ...
                    rolesWithContacts = state.roles.filter(role =>
                      (state.profiles[role] || []).some(p => state.selections[p.id]?.choice)
                    );
                  }
                    ...
                    return (
                      <div style={{
                        ...
                        <div style={{ fontSize: 14 }}>
                          {isFinal ? "Aucune décision de casting définitif" : "Aucun profil évalué par le réalisateur"}
                        </div>
                        <div style={{ fontSize: 12, color: "#555", marginTop: 6 }}>
                          {isFinal ? "Le réalisateur doit faire ses choix dans l'onglet Casting" : "Passez en Vue Réalisateur pour faire des sélections"}
                        </div>
                      </div>
                    );
                  }
                  ...
                  const roleProfiles = (state.profiles[currentRole] || []).map(p => ({
                    ...p, _role: currentRole, _selection: state.selections[p.id],
                  }));
                      ...
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                        ...
                        {availableProfiles.length === 0 && (
                          <div style={{ fontSize: 12, color: "#555", fontStyle: "italic" }}>
                            Aucun profil sélectionné disponible. Évaluez des profils en Vue Réalisateur.
                          </div>
                        )}
                      </div>
                    </div>
            ...
            <div>
              {(() => {
                ...
                if (allCasting.length === 0) {
                  return (
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 400, color: "#444" }}>
                      ...
                      <div style={{ fontSize: 12, color: "#555", marginTop: 6 }}>Faites d'abord des sélections en Vue Réalisateur</div>
                    </div>
                  );
                }
                ...
                const renderProfileCard = (profile, i) => {
                  ...
                  const sel = state.selections[profile.id];
                    ...
                    <div key={profile.id} style={{
                      ...
                      >
                        ...
                        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, flexShrink: 0 }}>
                          ...
                          {!isDirector && session.passStatus === "passed" && (
                      ...
                      {isDirector && finalSel?.selected != null && (
                        <div style={{
                          ...
                          <span style={{ fontSize: 11, fontWeight: 600, color: finalSel.selected ? "#22c55e" : "#ef4444" }}>
                            {finalSel.selected ? "🏆 Sélectionné par le réalisateur" : "✕ Non retenu"}
                          </span>
                          {finalSel.comment && <span style={{ fontSize: 11, color: "#888" }}>— « {finalSel.comment} »</span>}
                        </div>
                      )}

                      {/* === RÉALISATEUR: Notes display + Prominent choice === */}
                      {!isDirector && (
                        <>
                          {session.liveNotes && (
                  ...
                  <>
                    ...
                    <div style={{ marginBottom: 24 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 12, fontSize: 13, color: "#666" }}>
                        ...
                        {!isDirector && <span style={{ color: "#f59e0b" }}>· Vue Réalisateur</span>}
                      </div>
                    </div>
              ...
              {(() => {
                ...
                if (allEvaluated.length === 0) {
                  return (
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 400, color: "#444" }}>
                      ...
                      <div style={{ fontSize: 12, color: "#555", marginTop: 6 }}>Le réalisateur doit faire sa sélection dans l'onglet Casting</div>
                    </div>
                  );
                }
                ...
                return (
                  <>
                    ...
                    {/* Sections */}
                    {renderSection("Retenus par le réalisateur", "✓", "#22c55e", selected, "selected")}
            ...
            {filteredProfiles.length === 0 ? (
              <div style={{
                ...
                <div style={{ fontSize: 48, marginBottom: 16, opacity: 0.4 }}>◎</div>
                <div style={{ fontSize: 14, marginBottom: 8 }}>
                  {viewMode === "realisateur" ? "Aucun profil à évaluer pour ce rôle" : "Aucun profil pour ce rôle"}
                </div>
                {viewMode === "director" && (
                  <button
                    onClick={() => { setEditingProfile(null); setModalOpen(true); }}
              ...
              <>
                ...
                {(viewMode === "director" || viewMode === "realisateur") && (() => {
                  ...
                  const yC = currentProfiles.filter(p => state.selections[p.id]?.choice === "yes").length;
                  const mC = currentProfiles.filter(p => state.selections[p.id]?.choice === "maybe").length;
                  const nC = currentProfiles.filter(p => state.selections[p.id]?.choice === "no").length;
                ...
                <div style={{
                  ...
                  {filteredProfiles.map((profile, i) => (
                    <div key={profile.id} style={{ animation: `fadeIn 0.3s ease ${i * 0.05}s both`, position: "relative" }}>
                      ...
                      {viewMode === "realisateur" ? (
                        <RealisateurProfileCard
                          profile={profile}
                          selection={state.selections[profile.id]}
                          onSelect={(choice) => setSelection(profile.id, choice)}
                          onComment={(comment) => setSelectionComment(profile.id, comment)}
                        />
                      ) : (
                        <div style={{ position: "relative" }}>
                          <ProfileCard
                            ...
                            viewMode={viewMode}
                          />
                          {/* Director view: show réal's selection + contact status */}
                          {viewMode === "director" && state.selections[profile.id]?.choice && (
                            <div style={{ marginTop: 8, display: "flex", flexDirection: "column", gap: 6 }}>
                              <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                                <SelectionBadge selection={state.selections[profile.id]} />
                                <ContactStatusBadge contact={state.contacts[profile.id]} />
                              </div>
                              {state.selections[profile.id]?.comment && (
                                <div style={{
                              ...
                              {/* Contact button for selected profiles */}
                              {(state.selections[profile.id]?.choice === "yes" || state.selections[profile.id]?.choice === "maybe" || state.selections[profile.id]?.choice === "no") && (
                                <button
                                  ...
                                  style={{
                                    ...
                                    border: state.contacts[profile.id]?.status && state.contacts[profile.id]?.status !== "not_contacted"
                                      ? `1px solid ${CONTACT_STATUS[state.contacts[profile.id].status].color}44`
                                      : "1px solid #c9a44a44",
                                    background: state.contacts[profile.id]?.status && state.contacts[profile.id]?.status !== "not_contacted"
                                      ? CONTACT_STATUS[state.contacts[profile.id].status].bg
                                ...
                                >
                                  {state.contacts[profile.id]?.status && state.contacts[profile.id]?.status !== "not_contacted"
                                    ? "✏ Modifier le contact"
                                    : "✉ Contacter"
                                  }
                                </button>
                              ...
                              <div style={{ marginTop: 8, display: "flex", gap: 2 }}>
                                {[1,2,3,4,5].map(n => (
                                  <button key={n} onClick={() => quickRateProfile(profile.id, n)}
                                    style={{ width: 22, height: 22, borderRadius: 4, border: "none", cursor: "pointer", fontSize: 12,
                                      background: n <= (profile._quickRating || 0) ? "rgba(201,164,74,0.15)" : "rgba(255,255,255,0.02)",
                                      color: n <= (profile._quickRating || 0) ? "#c9a44a" : "#2a2a2e", transition: "all 0.15s",
                                    }}>★</button>
                                ))}
                              </div>
                              <div style={{ marginTop: 4, display: "flex", gap: 6, flexWrap: "wrap" }}>
                                {state.roles.length > 1 && (
                                  <button onClick={() => setMoveProfileModal({ profile, fromRole: activeRole })}
                                    style={{ padding: "4px 10px", background: "none", border: "1px solid #1e1e22", borderRadius: 6, color: "#555", fontSize: 10, cursor: "pointer", fontFamily: "inherit" }}
                                    onMouseEnter={e => e.currentTarget.style.color = "#c9a44a"}
                                    onMouseLeave={e => e.currentTarget.style.color = "#555"}>
                                    ↗ Déplacer
                                  </button>
              ...
              <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 60px)" }}>
                ...
                <div style={{ padding: "10px 16px", borderTop: "1px solid #1e1e22", display: "flex", alignItems: "center", gap: 8, flexShrink: 0, overflowX: "auto" }}>
                  ...
                  {state.roles.map(role => {
                    ...
                    const selected = profiles.filter(p => (state.selections[p.id]?.choice === "yes" || state.selections[p.id]?.choice === "maybe") && (p.email || p.agencyEmail));
                ...
                {profiles.map(p => (
                  <div key={p.id} style={{ background: "#111114", borderRadius: 14, border: "1px solid #1e1e22", overflow: "hidden" }}>
                    ...
                    <div style={{ padding: "14px" }}>
                      ...
                      {state.selections[p.id]?.choice && (
                        <div style={{ marginTop: 8 }}><SelectionBadge selection={state.selections[p.id]} /></div>
                      )}
      ...
      {shareModalOpen && state._shareCode && (
        <div onClick={() => { setShareModalOpen(false); setShareCopied(false); }} style={{ position: "fixed", inset: 0, zIndex: 9999, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <div onClick={e => e.stopPropagation()} style={{ width: "90%", maxWidth: 440, background: "#141416", borderRadius: 20, border: "1px solid #2a2a2e", padding: "32px", textAlign: "center" }}>
            ...
            <p style={{ fontSize: 13, color: "#888", marginBottom: 24, lineHeight: 1.5 }}>
              Le réalisateur ou la production peut rejoindre ce projet avec le code ci-dessous. Il verra les rôles, pourra faire ses sélections, et tout sera synchronisé.
            </p>
            <div style={{
              padding: "18px 24px", background: "#0c0c0e", borderRadius: 14, border: "2px dashed #c9a44a44",
              fontSize: 36, fontWeight: 800, letterSpacing: "0.25em", color: "#c9a44a",
              fontFamily: "'DM Sans', sans-serif", marginBottom: 16, userSelect: "all",
            }}>
              {state._shareCode}
            </div>
            <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
              <button onClick={() => {
                navigator.clipboard.writeText(state._shareCode);
                ...
                {shareCopied ? "✓ Code copié !" : "📋 Copier le code"}
              </button>
            </div>
            <div style={{ marginTop: 20, fontSize: 11, color: "#555", lineHeight: 1.6 }}>
              <strong style={{ color: "#999" }}>Comment ça marche :</strong><br />
              1. Envoyez le code au réalisateur<br />
              2. Il ouvre l'app et entre le code sur le dashboard<br />
              3. Il fait ses choix (oui/non/peut-être + commentaires)<br />
              4. Cliquez <span style={{ color: "#60a5fa" }}>🔄 Sync</span> pour récupérer ses choix
            </div>
          </div>
        </div>
      )}
...
export default function CastingApp() {
  ...
  // Check if site was already unlocked
  useEffect(() => {
    try {
      if (localStorage.getItem("_site_unlocked") === "yes") setSiteUnlocked(true);
    } catch {}
  }, []);
  ...
  const handleSiteUnlock = () => {
    if (siteCode.toUpperCase().trim() === SITE_ACCESS_CODE) {
      ...
      setSiteCodeError("");
      try { localStorage.setItem("_site_unlocked", "yes"); } catch {}
      // Auto-login — skip login screen
      const user = {
        id: "director_" + Date.now().toString(36),
        email: "casting@jojo.com",
  ...
  useEffect(() => {
    (async () => {
      try {
        const data = await window.storage.get("auth:session");
        if (data?.value) {
          const session = JSON.parse(data.value);
          setAuthUser(session);
          setAuthState("authenticated");
        } else if (siteUnlocked) {
          // Site unlocked but no session — auto-create one
          const user = {
            id: "director_" + Date.now().toString(36),
            email: "casting@jojo.com",
            firstName: "Directrice",
        ...
        if (siteUnlocked) {
          const user = { id: "director_fallback", email: "casting@jojo.com", firstName: "Directrice", lastName: "Casting", provider: "site_code", loggedAt: new Date().toISOString() };
          setAuthUser(user);
          setAuthState("authenticated");
        } else {
          setAuthState("login");
  ...
  const handleLogin = async (provider) => {
    ...
    if (provider === "google" || provider === "apple") {
      const user = {
        ...
        email: provider === "google" ? "vous@gmail.com" : "vous@icloud.com",
    ...
    try {
      const data = await window.storage.get(`user:${authForm.email.toLowerCase()}`);
      if (!data?.value) { setAuthError("Aucun compte avec cet email. Créez un compte."); return; }
      const stored = JSON.parse(data.value);
      if (stored.password !== authForm.password) { setAuthError("Mot de passe incorrect"); return; }
      const user = { ...stored, loggedAt: new Date().toISOString() };
      delete user.password;
      try { await window.storage.set("auth:session", JSON.stringify(user)); } catch (e) {}
      setAuthUser(user);
      setAuthState("authenticated");
    } catch (e) {
      // If storage totally fails, allow quick login as fallback
      const user = {
        id: Date.now().toString(36),
        email: authForm.email || "user@casting.com",
        firstName: authForm.firstName || authForm.email?.split("@")[0] || "Utilisateur",
        lastName: "",
        provider: "email",
        loggedAt: new Date().toISOString(),
      };
  ...
  const handleSignup = async () => {
    ...
    if (!authForm.email || !authForm.password || !authForm.firstName) {
      setAuthError("Tous les champs sont requis"); return;
    }
    if (authForm.password.length < 6) { setAuthError("Mot de passe : 6 caractères minimum"); return; }
    try {
      const existing = await window.storage.get(`user:${authForm.email.toLowerCase()}`);
      if (existing?.value) { setAuthError("Un compte existe déjà avec cet email"); return; }
    } catch (e) { /* storage might fail, continue anyway */ }
    const user = {
    ...
    try { await window.storage.set("auth:session", JSON.stringify(session)); } catch (e) {}
    setAuthUser(session);
    setAuthState("authenticated");
  };
  ...
  // Timeout on loading — if storage doesn't respond in 2s, go to login
  useEffect(() => {
        ...
        <div style={{ minHeight: "100vh", background: "#0a0a0c", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ width: "100%", maxWidth: 380, textAlign: "center", animation: "fadeIn 0.5s ease" }}>
            ...
            <p style={{ fontSize: 13, color: "#666", marginBottom: 32, lineHeight: 1.5 }}>
              Ce site est réservé à l'équipe de casting.<br/>Entrez le code d'accès pour continuer.
            </p>
            <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
              <input
                ...
                autoFocus
                style={{
                  flex: 1, padding: "14px 18px", background: "#111114", border: siteCodeError ? "1px solid rgba(239,68,68,0.4)" : "1px solid #2a2a2e",
                  borderRadius: 12, color: "#e0e0e0", fontSize: 16, fontFamily: "'DM Sans',sans-serif",
                  outline: "none", textAlign: "center", letterSpacing: "0.15em", fontWeight: 700,
                }}
                onFocus={e => e.target.style.borderColor = "#c9a44a"}
            ...
            <div style={{ marginTop: 40, fontSize: 10, color: "#333" }}>
              Jojo Casting · Espace professionnel
            </div>
          </div>
        </div>
      </>
      ...
      <>
        ...
        <div style={{ minHeight: "100vh", background: "#0a0a0c", display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ width: "100%", maxWidth: 420, animation: "fadeIn 0.5s ease" }}>
            ...
            <div style={{ textAlign: "center", marginBottom: 40 }}>
              <div style={{ fontSize: 48, marginBottom: 12 }}>🎬</div>
              <h1 style={{ fontSize: 32, fontWeight: 800, fontFamily: "'Playfair Display', serif", color: "#f0f0f0", marginBottom: 6 }}>
                Casting Director
              </h1>
              <p style={{ fontSize: 13, color: "#666", lineHeight: 1.5 }}>
                {authState === "login" ? "Connectez-vous pour accéder à vos projets" : "Créez votre compte"}
              </p>
            </div>

            {/* Card */}
            <div style={{ background: "#111114", borderRadius: 20, border: "1px solid #1e1e22", padding: "32px 28px", marginBottom: 16 }}>

              {authState === "login" ? (
                <>
                  {/* Email / Password */}
                  <div style={{ marginBottom: 12 }}>
                    <label style={{ fontSize: 10, color: "#555", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", display: "block", marginBottom: 5 }}>Email</label>
                    <input value={authForm.email} onChange={e => setAuthForm(p => ({ ...p, email: e.target.value }))}
                      ...
                      style={{ width: "100%", padding: "12px 14px", background: "#0c0c0e", border: "1px solid #2a2a2e", borderRadius: 10, color: "#e0e0e0", fontSize: 14, fontFamily: "'DM Sans',sans-serif", outline: "none" }}
                      onFocus={e => e.target.style.borderColor = "#c9a44a"} onBlur={e => e.target.style.borderColor = "#2a2a2e"}
                      onKeyDown={e => { if (e.key === "Enter") handleLogin("email"); }} />
                  </div>
                  <div style={{ marginBottom: 16 }}>
                    <label style={{ fontSize: 10, color: "#555", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", display: "block", marginBottom: 5 }}>Mot de passe</label>
                  ...
                  {authError && <div style={{ padding: "10px 14px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 8, color: "#ef4444", fontSize: 12, marginBottom: 14 }}>{authError}</div>}

                  <button onClick={() => handleLogin("email")}
                    style={{
                      width: "100%", padding: "14px", background: "linear-gradient(135deg, #c9a44a, #b8963a)",
                      border: "none", borderRadius: 12, cursor: "pointer", fontSize: 15, fontWeight: 700,
                      fontFamily: "inherit", color: "#000", transition: "all 0.2s",
                ...
                <>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
                    <div>
                      <label style={{ fontSize: 10, color: "#555", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", display: "block", marginBottom: 5 }}>Prénom *</label>
                      <input value={authForm.firstName} onChange={e => setAuthForm(p => ({ ...p, firstName: e.target.value }))}
                        placeholder="Marie" autoComplete="given-name"
                        style={{ width: "100%", padding: "12px 14px", background: "#0c0c0e", border: "1px solid #2a2a2e", borderRadius: 10, color: "#e0e0e0", fontSize: 14, fontFamily: "'DM Sans',sans-serif", outline: "none" }}
                        onFocus={e => e.target.style.borderColor = "#c9a44a"} onBlur={e => e.target.style.borderColor = "#2a2a2e"} />
                    </div>
                    <div>
                      <label style={{ fontSize: 10, color: "#555", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em", display: "block", marginBottom: 5 }}>Nom</label>
                      <input value={authForm.lastName} onChange={e => setAuthForm(p => ({ ...p, lastName: e.target.value }))}
            ...
            <div style={{ textAlign: "center" }}>
              {authState === "login" ? (
                <p style={{ fontSize: 13, color: "#666" }}>
                  ...
                  <span onClick={() => { setAuthState("signup"); setAuthError(""); }} style={{ color: "#c9a44a", fontWeight: 600, cursor: "pointer" }}>
                    Créer un compte
                  </span>
                </p>
              ) : (
                <p style={{ fontSize: 13, color: "#666" }}>
                  Déjà un compte ?{" "}
                  <span onClick={() => { setAuthState("login"); setAuthError(""); }} style={{ color: "#c9a44a", fontWeight: 600, cursor: "pointer" }}>
                    Se connecter
                  </span>
                </p>
              )}
            ...
            <div style={{ marginTop: 20, textAlign: "center" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 14 }}>
                ...
                <span style={{ fontSize: 10, color: "#444", fontWeight: 500 }}>accès rapide</span>
                <div style={{ flex: 1, height: 1, background: "#1e1e22" }} />
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                <input value={authForm.firstName} onChange={e => setAuthForm(p => ({ ...p, firstName: e.target.value }))}
                  placeholder="Votre prénom"
                  style={{ flex: 1, padding: "10px 14px", background: "#111114", border: "1px solid #1e1e22", borderRadius: 10, color: "#e0e0e0", fontSize: 13, fontFamily: "'DM Sans',sans-serif", outline: "none" }}
                  onFocus={e => e.target.style.borderColor = "#c9a44a"} onBlur={e => e.target.style.borderColor = "#1e1e22"}
                  onKeyDown={e => { if (e.key === "Enter") handleLogin("quick"); }}
                />
                <button onClick={() => handleLogin("quick")}
                  style={{
                    padding: "10px 20px", background: "rgba(201,164,74,0.08)", border: "1px solid rgba(201,164,74,0.2)",
                    borderRadius: 10, color: "#c9a44a", fontSize: 13, fontWeight: 700, cursor: "pointer",
              ...
              <div style={{ fontSize: 10, color: "#444", marginTop: 8 }}>Tapez votre prénom et entrez directement</div>
            </div>

            {/* Footer */}
            <div style={{ textAlign: "center", marginTop: 40, fontSize: 10, color: "#333" }}>
              Casting Director App · Propulsé par votre créativité
            </div>
          </div>
        </div>
      </>
  ...
  return <ErrorBoundary><CastingAppInner authUser={authUser} onLogout={handleLogout} /></ErrorBoundary>;
}
