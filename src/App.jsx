...
function CastingAppInner({ authUser }) {
  ...
  const projetDateChips = [
    { label: "Tournage", value: fmtDateFR(pi.dateTournage), icon: "\u{1F3AC}" },
    { label: "Rendu profils", value: fmtDateFR(pi.dateRenduProfils), icon: "\u{1F4E4}" },
    { label: "PPM", value: fmtDateFR(pi.datePPM), icon: "\u{1F4CB}" },
    ...(pi.castingDates || []).filter(d => d).map((d, i) => ({ label: "Casting " + (i + 1), value: fmtDateFR(d), icon: "\u{1F3AD}" })),
    { label: "Validation", value: fmtDateFR(pi.dateValidation), icon: "\u2705" },
    { label: "Répétition", value: fmtDateFR(pi.dateRepetition), icon: "\u{1F504}" },
  ].filter(x => x.value);
